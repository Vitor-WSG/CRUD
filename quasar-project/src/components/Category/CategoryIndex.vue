    ...
    <q-table title="teste" hide-pagination :rows="rows" :columns="columns">
      <template v-slot:top>
        ...
        <div class="col-xs-6" align="right">
          <q-btn color="primary" push label="Cadastrar" no-caps icon="add" @click="goToCreateCategory()"></q-btn>
        </div>
      ...
      <template v-slot:body-cell-action="props">
        <q-td :props="props">
          <div class="q-gutter-sm">
            ...
            <q-btn class="glossy" size="sm" no-caps push icon="delete" color="red" label="Excluir" @click="goToDeleteCategory()"/>
          </div>
        </q-td>
      </template>
    </q-table>

    <q-dialog v-model="createCategory" persistent >
      <q-card style="width: 700px; max-width: 80vw;">
        ...
        <q-card-section class="row items-center q-col-gutter-md">
          <div class="col-xs-4">
            <q-input outlined label="Código da categoria"></q-input>
          </div>
          <div class="col-xs-8">
            <q-input outlined label="Descrição da categoria"></q-input>
          </div>
        </q-card-section>
